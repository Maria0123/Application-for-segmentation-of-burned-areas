...
def train(args, snapshot_path):
    ...
    model_unsupervised_middle = create_model(with_stats = args.with_stats)
    model_unsupervised_ema = create_model(with_stats = args.with_stats, ema=True)
    ...
    model_unsupervised_middle.train()
    model_unsupervised_ema.train()

    valloader = DataLoader(db_val, batch_size=1, shuffle=False,
                           num_workers=0)

    optimizer1 = optim.AdamW(model_supervised.parameters(), lr=base_lr, weight_decay=0.01)
    optimizer2 = optim.AdamW(model_unsupervised_middle.parameters(), lr=base_lr, weight_decay=0.01)
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            outputs2 = model_unsupervised_middle(unlabeled_volume_batch)
            ...
            with torch.no_grad():
                ema_output = model_unsupervised_ema(ema_inputs)
            ...
            consistency_loss = torch.mean(
                    (unsupervised_pseudo_labels-ema_output_soft)**2)
            
            loss = model_supervised_loss + consistency_weight * (r_drop_loss + consistency_loss)
            ...
            update_ema_variables(model_unsupervised_middle, model_unsupervised_ema, args.ema_decay, iter_num)
            ...
            if args.with_stats:
                writeNetStats(model_supervised, model_unsupervised_middle, writer, iter_num)
            ...
            logging.info('iteration %d : supervised loss : %f unsupervised loss : %f r_drop_loss: %f consistency_loss %f' % (iter_num, model_supervised_loss.item(), consistency_weight * (r_drop_loss.item() + consistency_loss), r_drop_loss, consistency_loss))
            ...
            if iter_num % 50 == 0:
                ...
                writer.add_image('train/model_unsupervised_middle_Prediction',
                                 outputs[0, ...] * 50, iter_num)
            ...
            if iter_num > 0 and iter_num % 300 == 0:
                ...
                model_unsupervised_middle.eval()
                metric_list = 0.0
                for i_batch, sampled_batch in enumerate(valloader):
                    metric_i = test_single_volume_cbr(
                        sampled_batch["image"], sampled_batch["label"], model_unsupervised_middle, classes=num_classes)
                    metric_list += np.array(metric_i)
                
                metric_list = metric_list / len(db_val)
                performance2 = np.mean(metric_list, axis=0)

                writer.add_scalar('info/model_unsupervised_middle_val_mean_dice', performance2[0], iter_num)
                writer.add_scalar('info/model_unsupervised_middle_val_mean_hd95', performance2[1], iter_num)
                writer.add_scalar('info/model_unsupervised_middle_val_mean_jc', performance2[2], iter_num)
                writer.add_scalar('info/model_unsupervised_middle_val_mean_f1', performance2[3], iter_num)

                logging.info(
                    'model_unsupervised_middle iteration %d : mean_dice : %f mean_hd95 : %f mean_jc : %f mean_f1 : %f' % 
                ...
                if performance2 > best_performance2:
                    ...
                    save_mode_path = os.path.join(snapshot_path,
                                                  'model_unsupervised_middle_iter_{}_dice_{}.pth'.format(
                                                      iter_num, round(best_performance2, 4)))
                    save_best = os.path.join(snapshot_path,
                                             '{}_best_model_unsupervised_middle.pth'.format(args.model))
                    torch.save(model_unsupervised_middle.state_dict(), save_mode_path)
                    torch.save(model_unsupervised_middle.state_dict(), save_best)
            ...
            if iter_num % 3000 == 0:
                ...
                save_mode_path = os.path.join(
                    snapshot_path, 'model_unsupervised_middle_iter_' + str(iter_num) + '.pth')
                torch.save(model_unsupervised_middle.state_dict(), save_mode_path)
                logging.info("save model_unsupervised_middle to {}".format(save_mode_path))
