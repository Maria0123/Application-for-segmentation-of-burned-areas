...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            print(outputs1.sum(), outputs2.sum())

            consistency_weight = get_current_consistency_weight(iter_num)

            loss_ce_1 = ce_loss(outputs1, label_batch.squeeze())
            loss_dc_hd_1 = dc_hd_loss(outputs_soft1, label_batch)
            model1_loss = 0.5 * (loss_ce_1 + loss_dc_hd_1)

            loss_ce_2 = ce_loss(outputs2, label_batch.squeeze())
            loss_dc_hd_2 = dc_hd_loss(outputs_soft2, label_batch)
            model2_loss = 0.5 * (loss_ce_2 + loss_dc_hd_2)

            r_drop_loss = losses.compute_kl_loss(outputs1, outputs2)
